...
const City = () => {
  ...
  // const uniqueCity = [...new Set(propertiesData.map(item=> item.city))]

  const cityPropertiesCount = propertiesData.reduce((arr, property)=>{
    arr[property.city] = (arr[property.city] || 0) + 1

    return arr
  }, {})

  console.log(cityPropertiesCount)
  ...
  {Object.entries(cityPropertiesCount).map(([city, count])=>
    <Link to={`/properties?type=${(city).toLowerCase()}`} >
